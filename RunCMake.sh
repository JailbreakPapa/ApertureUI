...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --no-unity)
      NoUnity="-DNS_ENABLE_FOLDER_UNITY_FILES=OFF"
      shift 1
      ;;
...
if [ "$Distribution" = "Ubuntu" ] || [ "$Distribution" = "Mint" -a "$Version" = "21" ]; then
  packages=(cmake build-essential ninja-build libxrandr-dev libxinerama-dev libxcursor-dev libxi-dev uuid-dev mold libfreetype-dev libwayland-dev libssl-dev)
...
DisableFlags="-DGLFW_BUILD_TEST=OFF -DURIPARSER_BUILD_TESTS=OFF -DURIPARSER_BUILD_TOOLS=OFF -DURIPARSER_BUILD_DOCS=OFF -DNS_DEVELOPEMENT_ENABLE_SENTRY:BOOL=OFF"

if [ "$RunCMake" = true ]; then
  BuildDir="build"
  cmake -B $BuildDir -S . -G Ninja $DisableFlags  -DCMAKE_CXX_COMPILER=$cxx_compiler -DNS_ENABLE_QT_SUPPORT=OFF -DCMAKE_C_COMPILER=$c_compiler -DNS_EXPERIMENTAL_EDITOR_ON_LINUX=ON -DNS_BUILD_EXPERIMENTAL_VULKAN=ON -DCMAKE_BUILD_TYPE=$BuildType -DCMAKE_EXPORT_COMPILE_COMMANDS=ON $NoUnity -DCURL_USE_OPENSSL=OFF && \
